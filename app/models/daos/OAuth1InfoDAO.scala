...
class OAuth1InfoDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def oAuth1InfoQuery(loginInfo: LoginInfo): Query[OAuth1Infos, DBOAuth1Info, Seq] = for {
    dbLoginInfo <- loginInfoQuery(loginInfo)
    dbOAuth1Info <- slickOAuth1Infos if dbOAuth1Info.loginInfoId === dbLoginInfo.id
  } yield dbOAuth1Info

  // Use sub-query workaround instead of join to get auth-info because slick only supports selecting
  // from a single table for update/delete queries (https://github.com/slick/slick/issues/684).
  protected def oAuth1InfoSubQuery(loginInfo: LoginInfo): Query[OAuth1Infos, DBOAuth1Info, Seq] =
    slickOAuth1Infos.filter(_.loginInfoId in loginInfoQuery(loginInfo).map(_.id))

  protected def addAction(loginInfo: LoginInfo, authInfo: OAuth1Info): DBIOAction[Int, NoStream, Effect.Read with Effect.Write with Effect.Transactional] =
