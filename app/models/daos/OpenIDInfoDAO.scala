...
class OpenIDInfoDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def openIDInfoQuery(loginInfo: LoginInfo): Query[OpenIDInfos, DBOpenIDInfo, Seq] = for {
    dbLoginInfo <- loginInfoQuery(loginInfo)
    dbOpenIDInfo <- slickOpenIDInfos if dbOpenIDInfo.loginInfoId === dbLoginInfo.id
  } yield dbOpenIDInfo

  protected def addAction(loginInfo: LoginInfo, authInfo: OpenIDInfo): DBIOAction[Unit, NoStream, Effect.Read with Effect.Write with Effect.Transactional] =
