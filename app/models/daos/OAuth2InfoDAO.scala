...
class OAuth2InfoDAO @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def oAuth2InfoQuery(loginInfo: LoginInfo): Query[OAuth2Infos, DBOAuth2Info, Seq] = for {
  ...
  protected def oAuth2InfoSubQuery(loginInfo: LoginInfo): Query[OAuth2Infos, DBOAuth2Info, Seq] =
    slickOAuth2Infos.filter(_.loginInfoId in loginInfoQuery(loginInfo).map(_.id))

  protected def addAction(loginInfo: LoginInfo, authInfo: OAuth2Info): DBIOAction[Int, NoStream, Effect.Read with Effect.Write with Effect.Transactional] =
